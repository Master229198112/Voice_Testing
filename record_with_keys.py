...
def record_audio(file_name):
    ...
    print(f"Recording {file_name}. Press SPACE to stop.")

    while not is_pressed("space"):  # Wait for spacebar to stop recording
    ...
    # Save the audio file
    os.makedirs("recordings/test", exist_ok=True)
    file_path = os.path.join("recordings/test", file_name + ".wav")
...
def main():
    ...
    print("Press SPACE to start/stop recording. Press ESC after each recording to proceed to the next.")
    for file_name, content in texts.items():
        print(f"\nPlease read: {content}")
        print(f"Press SPACE to start recording for '{file_name}'.")
        
        # Wait for SPACE to start recording
        while not is_pressed("space"):
            pass
        while is_pressed("space"):  # Avoid multiple triggers for a single press
            pass

        # Start recording
        record_audio(file_name)

        print(f"Recording for '{file_name}' completed. Press ESC to continue or SPACE to re-record.")
        while not is_pressed("esc"):
            if is_pressed("space"):
                print(f"Re-recording '{file_name}'...")
                record_audio(file_name)
                print(f"Re-recording for '{file_name}' completed.")
                print("Press ESC to continue or SPACE to re-record again.")
                while is_pressed("space"):
                    pass

        while is_pressed("esc"):  # Avoid multiple triggers for a single press
            pass
